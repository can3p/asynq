...
var _ base.Broker = (*RDBRueidis)(nil)
...
type RDBRueidis struct {
	origClient rueidis.Client
	client     rueidiscompat.Cmdable
	clock      timeutil.Clock
}

// NewRDB returns a new instance of RDB.
func NewRDB(client rueidis.Client) *RDBRueidis {
...
func (r *RDBRueidis) SetClock(c timeutil.Clock) {
	r.clock = c
}

// Ping checks the connection with redis server.
func (r *RDBRueidis) Ping() error {
	return r.client.Ping(context.Background()).Err()
}

func (r *RDBRueidis) runScript(ctx context.Context, op errors.Op, script *rueidiscompat.Script, keys []string, args ...interface{}) error {
...
func (r *RDBRueidis) runScriptWithErrorCode(ctx context.Context, op errors.Op, script *rueidiscompat.Script, keys []string, args ...interface{}) (int64, error) {
...
var enqueueCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) Enqueue(ctx context.Context, msg *base.TaskMessage) error {
	...
	n, err := r.runScriptWithErrorCode(ctx, op, enqueueCmdCompat, keys, argv...)
	if err != nil {
		return err
	}
	if n == 0 {
...
var enqueueUniqueCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) EnqueueUnique(ctx context.Context, msg *base.TaskMessage, ttl time.Duration) error {
	...
	n, err := r.runScriptWithErrorCode(ctx, op, enqueueUniqueCmdCompat, keys, argv...)
	if err != nil {
		return err
	}
	if n == -1 {
...
var dequeueCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) Dequeue(qnames ...string) (msg *base.TaskMessage, leaseExpirationTime time.Time, err error) {
	...
	for _, qname := range qnames {
		...
		res, err := dequeueCmdCompat.Run(context.Background(), r.client, keys, argv...).Result()
...
var doneCmdCompat = rueidiscompat.NewScript(`
...
// ARGV[2] -> stats expiration timestamp
// ARGV[3] -> max int64 value
var doneUniqueCmdCompat = rueidiscompat.NewScript(`
if redis.call("LREM", KEYS[1], 0, ARGV[1]) == 0 then
  return redis.error_reply("NOT FOUND")
end
if redis.call("ZREM", KEYS[2], ARGV[1]) == 0 then
...
func (r *RDBRueidis) Done(ctx context.Context, msg *base.TaskMessage) error {
	...
	if len(msg.UniqueKey) > 0 {
		...
		return r.runScript(ctx, op, doneUniqueCmdCompat, keys, argv...)
	}
	return r.runScript(ctx, op, doneCmdCompat, keys, argv...)
}
...
var markAsCompleteCmdCompat = rueidiscompat.NewScript(`
if redis.call("LREM", KEYS[1], 0, ARGV[1]) == 0 then
  return redis.error_reply("NOT FOUND")
end
if redis.call("ZREM", KEYS[2], ARGV[1]) == 0 then
...
// ARGV[4] -> task message data
// ARGV[5] -> max int64 value
var markAsCompleteUniqueCmdCompat = rueidiscompat.NewScript(`
if redis.call("LREM", KEYS[1], 0, ARGV[1]) == 0 then
  return redis.error_reply("NOT FOUND")
end
if redis.call("ZREM", KEYS[2], ARGV[1]) == 0 then
...
// It removes a uniqueness lock acquired by the task, if any.
func (r *RDBRueidis) MarkAsComplete(ctx context.Context, msg *base.TaskMessage) error {
	...
	if len(msg.UniqueKey) > 0 {
		keys = append(keys, msg.UniqueKey)
		return r.runScript(ctx, op, markAsCompleteUniqueCmdCompat, keys, argv...)
	}
	return r.runScript(ctx, op, markAsCompleteCmdCompat, keys, argv...)
}
...
var requeueCmdCompat = rueidiscompat.NewScript(`
if redis.call("LREM", KEYS[1], 0, ARGV[1]) == 0 then
  return redis.error_reply("NOT FOUND")
end
if redis.call("ZREM", KEYS[2], ARGV[1]) == 0 then
...
func (r *RDBRueidis) Requeue(ctx context.Context, msg *base.TaskMessage) error {
	...
	return r.runScript(ctx, op, requeueCmdCompat, keys, msg.ID)
}
...
// Returns 0 if task ID already exists
var addToGroupCmdCompat = rueidiscompat.NewScript(`
if redis.call("EXISTS", KEYS[1]) == 1 then
	return 0
end
redis.call("HSET", KEYS[1],
...
func (r *RDBRueidis) AddToGroup(ctx context.Context, msg *base.TaskMessage, groupKey string) error {
	...
	n, err := r.runScriptWithErrorCode(ctx, op, addToGroupCmdCompat, keys, argv...)
	if err != nil {
		return err
	}
	if n == 0 {
...
// Returns 0 if task ID already exists
// Returns -1 if task unique key already exists
var addToGroupUniqueCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) AddToGroupUnique(ctx context.Context, msg *base.TaskMessage, groupKey string, ttl time.Duration) error {
	...
	n, err := r.runScriptWithErrorCode(ctx, op, addToGroupUniqueCmdCompat, keys, argv...)
	if err != nil {
		return err
	}
	if n == -1 {
...
// Returns 1 if successfully enqueued
// Returns 0 if task ID already exists
var scheduleCmdCompat = rueidiscompat.NewScript(`
if redis.call("EXISTS", KEYS[1]) == 1 then
	return 0
end
redis.call("HSET", KEYS[1],
...
func (r *RDBRueidis) Schedule(ctx context.Context, msg *base.TaskMessage, processAt time.Time) error {
	...
	n, err := r.runScriptWithErrorCode(ctx, op, scheduleCmdCompat, keys, argv...)
	if err != nil {
		return err
	}
	if n == 0 {
...
// Returns 0 if task ID already exists
// Returns -1 if task unique key already exists
var scheduleUniqueCmdCompat = rueidiscompat.NewScript(`
local ok = redis.call("SET", KEYS[1], ARGV[1], "NX", "EX", ARGV[2])
if not ok then
  return -1
end
...
// It returns ErrDuplicateTask if the lock cannot be acquired.
func (r *RDBRueidis) ScheduleUnique(ctx context.Context, msg *base.TaskMessage, processAt time.Time, ttl time.Duration) error {
	...
	n, err := r.runScriptWithErrorCode(ctx, op, scheduleUniqueCmdCompat, keys, argv...)
	if err != nil {
		return err
	}
	if n == -1 {
...
var retryCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) Retry(ctx context.Context, msg *base.TaskMessage, processAt time.Time, errMsg string, isFailure bool) error {
	...
	return r.runScript(ctx, op, retryCmdCompat, keys, argv...)
}
...
var archiveCmdCompat = rueidiscompat.NewScript(`
if redis.call("LREM", KEYS[2], 0, ARGV[1]) == 0 then
  return redis.error_reply("NOT FOUND")
end
if redis.call("ZREM", KEYS[3], ARGV[1]) == 0 then
...
func (r *RDBRueidis) Archive(ctx context.Context, msg *base.TaskMessage, errMsg string) error {
	...
	return r.runScript(ctx, op, archiveCmdCompat, keys, argv...)
}
...
func (r *RDBRueidis) ForwardIfReady(qnames ...string) error {
...
var forwardCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) forward(delayedKey, pendingKey, taskKeyPrefix, groupKeyPrefix string) (int, error) {
	now := r.clock.Now()
	...
	res, err := forwardCmdCompat.Run(context.Background(), r.client, keys, argv...).Result()
	if err != nil {
...
func (r *RDBRueidis) forwardAll(qname string) (err error) {
...
func (r *RDBRueidis) ListGroups(qname string) ([]string, error) {
...
var aggregationCheckCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) AggregationCheck(qname, gname string, t time.Time, gracePeriod, maxDelay time.Duration, maxSize int) (string, error) {
	...
	n, err := r.runScriptWithErrorCode(context.Background(), op, aggregationCheckCmdCompat, keys, argv...)
	if err != nil {
		return "", err
	}
	switch n {
...
var readAggregationSetCmdCompat = rueidiscompat.NewScript(`
local msgs = {}
...
func (r *RDBRueidis) ReadAggregationSet(qname, gname, setID string) ([]*base.TaskMessage, time.Time, error) {
	var op errors.Op = "RDB.ReadAggregationSet"
	ctx := context.Background()
	aggSetKey := base.AggregationSetKey(qname, gname, setID)
	res, err := readAggregationSetCmdCompat.Run(ctx, r.client,
...
var deleteAggregationSetCmdCompat = rueidiscompat.NewScript(`
local ids = redis.call("ZRANGE", KEYS[1], 0, -1)
for _, id in ipairs(ids)  do
...
func (r *RDBRueidis) DeleteAggregationSet(ctx context.Context, qname, gname, setID string) error {
	var op errors.Op = "RDB.DeleteAggregationSet"
	keys := []string{
		base.AggregationSetKey(qname, gname, setID),
		base.AllAggregationSets(qname),
	}
	return r.runScript(ctx, op, deleteAggregationSetCmdCompat, keys, base.TaskKeyPrefix(qname))
}

// KEYS[1] -> asynq:{<qname>}:aggregation_sets
// -------
// ARGV[1] -> current time in unix time
var reclaimStateAggregationSetsCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) ReclaimStaleAggregationSets(qname string) error {
	var op errors.Op = "RDB.ReclaimStaleAggregationSets"
	return r.runScript(context.Background(), op, reclaimStateAggregationSetsCmdCompat,
...
var deleteExpiredCompletedTasksCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) DeleteExpiredCompletedTasks(qname string, batchSize int) error {
	for {
...
func (r *RDBRueidis) deleteExpiredCompletedTasks(qname string, batchSize int) (int64, error) {
	...
	res, err := deleteExpiredCompletedTasksCmdCompat.Run(context.Background(), r.client, keys, argv...).Result()
	if err != nil {
...
// ARGV[2] -> task key prefix
var listLeaseExpiredCmdCompat = rueidiscompat.NewScript(`
local res = {}
...
func (r *RDBRueidis) ListLeaseExpired(cutoff time.Time, qnames ...string) ([]*base.TaskMessage, error) {
	var op errors.Op = "rdb.ListLeaseExpired"
	var msgs []*base.TaskMessage
	for _, qname := range qnames {
		res, err := listLeaseExpiredCmdCompat.Run(context.Background(), r.client,
...
func (r *RDBRueidis) ExtendLease(qname string, ids ...string) (expirationTime time.Time, err error) {
	expireAt := r.clock.Now().Add(LeaseDuration)
	var zs []rueidiscompat.Z
	for _, id := range ids {
		zs = append(zs, rueidiscompat.Z{Member: id, Score: float64(expireAt.Unix())})
	}
...
var writeServerStateCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) WriteServerState(info *base.ServerInfo, workers []*base.WorkerInfo, ttl time.Duration) error {
	...
	if err := r.client.ZAdd(ctx, base.AllServers, rueidiscompat.Z{Score: float64(exp.Unix()), Member: skey}).Err(); err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "sadd", Err: err})
	}
	if err := r.client.ZAdd(ctx, base.AllWorkers, rueidiscompat.Z{Score: float64(exp.Unix()), Member: wkey}).Err(); err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "zadd", Err: err})
	}
	return r.runScript(ctx, op, writeServerStateCmdCompat, []string{skey, wkey}, args...)
}

// KEYS[1] -> asynq:servers:{<host:pid:sid>}
// KEYS[2] -> asynq:workers:{<host:pid:sid>}
var clearServerStateCmdCompat = rueidiscompat.NewScript(`
redis.call("DEL", KEYS[1])
redis.call("DEL", KEYS[2])
return redis.status_reply("OK")`)

// ClearServerState deletes server state data from redis.
func (r *RDBRueidis) ClearServerState(host string, pid int, serverID string) error {
	...
	return r.runScript(ctx, op, clearServerStateCmdCompat, []string{skey, wkey})
}

// KEYS[1]  -> asynq:schedulers:{<schedulerID>}
// ARGV[1]  -> TTL in seconds
// ARGV[2:] -> schedler entries
var writeSchedulerEntriesCmdCompat = rueidiscompat.NewScript(`
redis.call("DEL", KEYS[1])
for i = 2, #ARGV do
...
func (r *RDBRueidis) WriteSchedulerEntries(schedulerID string, entries []*base.SchedulerEntry, ttl time.Duration) error {
	...
	err := r.client.ZAdd(ctx, base.AllSchedulers, rueidiscompat.Z{Score: float64(exp.Unix()), Member: key}).Err()
	if err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "zadd", Err: err})
	}
	return r.runScript(ctx, op, writeSchedulerEntriesCmdCompat, []string{key}, args...)
}

// ClearSchedulerEntries deletes scheduler entries data from redis.
func (r *RDBRueidis) ClearSchedulerEntries(scheduelrID string) error {
...
func (r *RDBRueidis) CancelationPubSub() (*redis.PubSub, error) {
...
func (r *RDBRueidis) PublishCancelation(id string) error {
...
var recordSchedulerEnqueueEventCmdCompat = rueidiscompat.NewScript(`
...
func (r *RDBRueidis) RecordSchedulerEnqueueEvent(entryID string, event *base.SchedulerEnqueueEvent) error {
	...
	return r.runScript(ctx, op, recordSchedulerEnqueueEventCmdCompat, keys, argv...)
}

// ClearSchedulerHistory deletes the enqueue event history for the given scheduler entry.
func (r *RDBRueidis) ClearSchedulerHistory(entryID string) error {
...
func (r *RDBRueidis) WriteResult(qname, taskID string, data []byte) (int, error) {
